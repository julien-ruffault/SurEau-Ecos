...
params <- c('LAImax','P50_VC_TL','gmin20','kPlantInit','SWC')
R <- 10^3
type <- "norm"
conf <- 0.95
...
(soilFile$saturation_capacity_vg-soilFile$residual_capacity_vg)
...
soil_parameters  <- create.soil.parameters(listOfParameters = soilFile, depths = DEPTH[i]*1/3 ,DEPTH[i]*2/3,DEPTH[i])
