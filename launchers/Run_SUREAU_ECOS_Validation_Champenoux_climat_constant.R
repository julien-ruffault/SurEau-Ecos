...
# ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ##
# Launcher to run SUREAU-ECOS (V2.1) on Champenoux for validation 
...
# Initialisation --------------------------------------------------------
#graphics.off()  # Clear graphic windows
rm(list = ls()) # Clear environement
gc()            # Clear memory
setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) # set main directory as the directory of this r script (launcher directory)
library(tictoc)
# Source functions and libraries------------------------------------------------
source("../functions/loadFunctionsSUREAUV2_1.R")

vectimestep=c(12,8,6,4,2,1)

#quartz()
tstep=1
# Define general parameters ----------------------------------------------------
general_params <- create.general.params(start_year         = 1990,                            # starting year  //note entire years only
                                        end_year           = 1990,                            # ending year
                                        ...
                                        outputType         = 'ValidICOS_simple',              # output type (hard coded, different options available, see with JR)
                                        timeStepForEvapo   =  tstep,                               # timeStep for the evaptranspiration module (default = 1)
...
#soil_params <- createSoilParams(general_params) 
soil_params <- createSoilParams(general_params,depths = c(0.373333 ,0.746666,1.119)) 
#soil_params <- createSoilParams(general_params,depths = c(0.43 ,0.86,1.29))   # !!!! default depth in the model is c(0.3,1,4) / here is peculiar case to match with SUREAU.C version 
# load vegetation parameters ---------------------------------------------------
veg_params  <- createVegParams(general_params)

# veg_params$PsiStartClosing <- (-2.)
# veg_params$PsiClose <- (-2.7)
# veg_params$P50_VC<- (-7)
...
toc()
warnings()
filename  = paste0("../Results_model/Champenoux_tests_climat_cst.csv");DATA = fread(filename);DD=strftime(max(DATA$Time), format="%j");datmax=max(DATA$Time);datmin=min(DATA$Time)
