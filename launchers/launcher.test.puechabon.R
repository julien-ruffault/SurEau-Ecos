# ### ### ### ### ### ### #s## ### ### ### ### ### ### ### ### ### ### ### ### #
# Test Launcher to run SurEau-ECOS (V4.0) on Puechabon
...
climateData_path          <- paste0(mainDir,'/datasets/Clim_data/Puechabon/Climat_Puechabon.csv') # <-- indicate here the path to input climate data 
soilParameters_path       <- paste0(mainDir,'/datasets/Soil_Parameters/Puechabon/Soil_Puechabon.csv')
vegetationParameters_path <- paste0(mainDir,'/datasets/Species_Parameters/Parameters_Quercus_ilex.csv')
#standParameters_path      <- paste0(mainDir,'datasets/test_data/stand_champenoux_test.csv')  
output_path               <- paste0(mainDir,'/Results_model/test_puech.csv')        
...
climate_data <- create.climate.data(filePath=climateData_path, modeling_options=modeling_options, 
...
# for analyses / 
  ...
  Tr=tapply(DATA$AET.C,yday(as.Date(DATA$DD)), sum)
  doy=unique(yday(as.Date(DATA$DD)))
  quartz()
  par(mfrow=c(2,2))
  plot(doy,Tr, type='l',col='firebrick1',ylim=c(0, 3))
