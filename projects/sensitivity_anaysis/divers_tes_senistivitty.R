...
#
#params=c("kPlantInit",'gsMax','betaRootProfile','LAImax','gmin20','Tair','RHair')

params=c("kPlantInit",'gsMax','LAImax')

N <- 200 # number for initial sampling / about 30000 for full models
k <- length(params) # number of parameters
#params=c('gsMax',"PiFullTurgor_Leaf")


R <- 10^3
type <- "norm"
conf <- 0.95
PARAMS= sobol_matrices(params=params,N=N,order="second")
...
#PARAMS[,"kPlantInit"] <-  qunif(PARAMS[, "kPlantInit"], vegFile$kPlantInit-0.8*vegFile$kPlantInit,vegFile$kPlantInit+0.8*vegFile$kPlantInit)

PARAMS[,"kPlantInit"] <-  qunif(PARAMS[, "kPlantInit"], 0.2,5)
...
PARAMS[, "gsMax"] <- qunif(PARAMS[, "gsMax"], vegFile$gsMax-0.9*vegFile$gsMax,vegFile$gsMax+0.9*vegFile$gsMax)
#PARAMS[, "P50_gs"] <- qunif(PARAMS[, "P50_gs"],vegFile$P50_gs+vegFile$P50_gs*0.5, vegFile$P50_gs-vegFile$P50_gs*0.5)
#PARAMS[,"PiFullTurgor_Leaf"] <-  qunif(PARAMS[, "PiFullTurgor_Leaf"], vegFile$PiFullTurgor_Leaf+0.5*vegFile$PiFullTurgor_Leaf,vegFile$PiFullTurgor_Leaf-0.5*vegFile$PiFullTurgor_Leaf)
#PARAMS[,"betaRootProfile"] <-  qunif(PARAMS[, "betaRootProfile"], 0.95,0.99)

#PARAMS[,"RHair"] <-  qunif(PARAMS[, "RHair"], 42.5,82.5)
...
foreach(i=1:nrow(PARAMS),.packages=c('lubridate','insol')) %dopar% {
  ...
  #climate_data$Tair_mean =35
  #climate_data$Tair_min =climate_data$Tair_mean-7.5
  #climate_data$Tair_max =climate_data$Tair_mean+7.5
  ...
  #vegFile$PiFullTurgor_Trunk = PARAMS[,"PiFullTurgor_Leaf"][i]
...
plot(ind)
ind <- sobol_indices(Y=Y1,N=N,params=params,order='second')
plot(ind)
